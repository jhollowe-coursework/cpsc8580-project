...
DOCUMENTATION = '''
    ...
    version_added: "1.2.0"
    author:
        - John Hollowell <john@johnhollowell.com>
    requirements:
        - requests >= 1.1
    description:
...
class InventoryModule(BaseInventoryPlugin):
    ...
    def _cloudlab_login(self):
        ...
        data = {"uid": self.get_option(
            "user"), "password": self.get_option("password"), "login": ""}

        r = self._get_session().post(url=login_url, data=data)
    ...
    def _get_element_children(self, node, tag):
        ...
        return [elem for elem in node if elem.tag.rpartition("}")[
            2] == tag]
    ...
    def _populate(self):
        ...
        data = {"ajax_route": "status",
                "ajax_method": "GetInstanceManifest",
                "ajax_args[uuid]": self.get_option("uuid")}

        r = self._get_session().post(url=data_url, data=data)
        ...
        for location in r_json:
            ...
            m = re.match(
                r'urn:publicid:IDN\+(.*)\+authority\+cm', location)
            if m:
            ...
            for node in nodes:
                ...
                if node.attrib.get("component_manager_id") == location:
                    ...
                    if self.get_option("group_per_physical_core"):
                        ...
                        m = re.match(
                            r'urn:publicid:IDN\+(.*)\+node\+(.*)', node.attrib.get("component_id"))
                        if m:
                            node_location = m.group(1)
                            ...
                            for login in self._get_element_children(service, "login"):
                                ...
                                if login.attrib["username"] == self.get_option("user"):
                                    self.inventory.set_variable(
                                        node_name, "ansible_user", login.attrib["username"])
                                    self.inventory.set_variable(
                                        node_name, "ansible_host", login.attrib["hostname"])
                                    self.inventory.set_variable(
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        # self.cloudlab_url_base = "https://webhook.site/2e500548-1c82-4f13-8f6e-27ec7fb8da0c".rstrip(
        # '/')
